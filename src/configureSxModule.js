define([
	...
	'fontoxml-families/createBulletingWidget',
	...
	return function configureSxModule (sxModule) {
		...
		configureAsFrameWithBlock(sxModule, 'self::lcGapItemDelta', t('gap item delta'), {
			emptyElementPlaceholderText: t('describe the gap between the learning objective and the task analysis'),
			blockHeaderLeft: [
				createMarkupLabelWidget()
			]
		});
		...
		configureAsFrame(sxModule, 'self::lcNeedsAnalysis', t('needs analysis'), {
			contextualOperations: [
				...
				{ name: ':lcNeedsAnalysis-insert-lcTask', hideIn: ['element-menu', 'breadcrumbs-menu'] },
		...
		//     applied.
		configureAsFrameWithBlock(sxModule, 'self::lcWorkEnvDescription', t('work environment description'), {
		...
		configureAsFrame(sxModule, 'self::learningPlan and ancestor::*[fonto:dita-class(., "topic/topic")]', undefined, {
			defaultTextContainer: 'learningPlanbody',
			blockFooter: [
				createRelatedNodesQueryWidget('./related-links')
			],
